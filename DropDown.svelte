<script>
    ...
    const dispatchEvent = (indexSelectedItem) => {
        dispatch("dropdownChanged", {index: indexSelectedItem, value: config.items[indexSelectedItem]});
    }
...
<select bind:value={selectedItem}>
    {#each config.items as selectItem, index}
        {#if index === initSelectedItem}
            <option value="{index}" selected>{selectItem.label || selectItem}</option>
        {:else}
...
<style lang="scss">
  ...
  select {
    ...
    background: url("data:image/svg+xml;charset=utf-8,%3Csvg class='stroke-current' width='16' height='16' fill='none' xmlns='http://www.w3.org/2000/svg' aria-hidden='true'%3E%3Cpath d='M12 6l-4 4-4-4' stroke-linecap='round' stroke='%23000'/%3E%3C/svg%3E") no-repeat center right 16px;
    ...
    @media (prefers-color-scheme: dark) {
      color: #F1EFED;
      background: url("data:image/svg+xml;charset=utf-8,%3Csvg class='stroke-current' width='16' height='16' fill='none' xmlns='http://www.w3.org/2000/svg' aria-hidden='true'%3E%3Cpath d='M12 6l-4 4-4-4' stroke-linecap='round' stroke='%23F1EFED'/%3E%3C/svg%3E") no-repeat center right 16px;
