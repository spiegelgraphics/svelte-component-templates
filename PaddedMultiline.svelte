<script>
  ...
  $: {
    ...
    if (unit !== unit2) {
      console.warn("padded-multiline: padding-right und padding-left müssen die gleiche Einheit haben. padding-left wird auf padding-right gesetzt.");
      valLeft = valRight;
    }
  ...
  $: styles = Object.entries(pmOptions).map(([key, value]) => `--${key}: ${value};`).join(" ");

</script>


<span class="padded-multiline" style="{styles}">
  <span class="text">
    <slot/>
  </span>
</span>
...
<style lang="scss">
  ...
  .padded-multiline {
    ...
    padding: var(--padding-top, 0.2rem) var(--padding-right, 0.5rem) var(--padding-bottom, 0.2rem) var(--padding-left, 0.5rem);

    $lightColor: rgba(255, 255, 255, 0.9);
    $darkColor: rgba(0, 0, 0, 0.9);

    --pml-backgroundColor: var(--background_lm, var(--color_dm, var(--background, #{$darkColor})));
    --pml-color: var(--color_lm, var(--background_dm, var(--color, #{$lightColor})));
    @media screen and (prefers-color-scheme: dark) {
      --pml-backgroundColor: var(--background_dm, var(--color_lm, var(--color, #{$lightColor})));
      --pml-color: var(--color_dm, var(--background_lm, var(--background, #{$darkColor})));
    }
